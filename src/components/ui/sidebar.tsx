...
>(({ className, isCollapsed, ...props }, ref) => {
  return (
    <div
      ...
      data-sidebar="footer"
      className={cn(
        "flex flex-col gap-2 p-4 mt-auto border-t",
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex w-full items-center gap-3 overflow-hidden rounded-md px-3 py-2 text-left text-sm outline-none ring-ring transition-all hover:bg-accent hover:text-accent-foreground focus-visible:ring-2 active:bg-accent active:text-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-[[data-sidebar=menu-action]]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-accent data-[active=true]:font-medium data-[active=true]:text-accent-foreground data-[state=open]:bg-accent data-[state=open]:text-accent-foreground",
  {
    variants: {
      variant: {
...
>(
  (
    ...
    const isActuallyCollapsed = isCollapsed ?? state === "collapsed";
    ...
    const button = (
      <Comp
        ...
        className={cn(sidebarMenuButtonVariants({ variant, size, isCollapsed: isActuallyCollapsed }), className)}
        {...props}
      />
    )
